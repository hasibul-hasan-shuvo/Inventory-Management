...
class ProductOutItemEditDialogContentView extends StatelessWidget
  ...
  Widget _getAvailableWithTitleView() {
    return Obx(
      () => Text(
        "${appLocalization.titleEditProductOutAvailableCount}: $itemLeftCount",
  ...
  Widget _getIncrementButton() {
    return Obx(
      () => IconButton(
        onPressed: itemLeftCount > 0 ? _onTapIncrement : null,
        icon: AssetImageView(
          fileName: AppIcons.roundedPlus,
          height: AppValues.iconLargeSize.h,
          width: AppValues.iconLargeSize.h,
          color: itemLeftCount > 0
              ? theme.colorScheme.primary
              : AppColors.basicGrey,
        ),
      ),
  ...
  int get itemLeftCount => data.available - _numberController.value;
}
